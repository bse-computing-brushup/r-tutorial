...
Rstudio has loads of useful shortcuts that you should explore that allow you to use the environment more efficiently. Among others, there are useful shortcuts CTRL+1 and CTRL+2. CTRL+1 moves the cursor to the script view while CTRL+2 moves the cursor to the console. Another useful shortcut is CTRL+SHIFT+C that comments and uncomments the code. 
...
Some other mathematical operations are also immediately available, e.g. sinus function or square root
...
You can also use the equality sign `=` to assign a value to a variable, but keep in mind that there is a subtle difference between these two assignment operators. "The operator `<-` can be used anywhere, whereas the operator `=` is only allowed at the top level (e.g., in the complete expression typed at the command prompt) or as one of the subexpressions in a braced list of expressions." -- you can get this explanation (an more on assignment operators) by typing `help('<-')` in the R Console. Particularly, you can use "<-" when you want an assignment statement to be executed and the value to be passed to a function at the same time, e.g. `mean(x <- 1:10)`. 
...
Here we set up an example with the package `dplyr`. This package has useful functions for getting quick summaries on variables in data frames, but for now we are only interested in installing and loading it.
...
In R it is crucial to operate on data structures (and not looping through them, operating element-by-element). It means that you should always try to "vectorize" operations as much as possible, e.g. by using means of vector-oriented arithmetics that R provides.
...
Note that you can give names to columns and rows of matrix assigning specific vectors to `colnames(matrix)` and `rownames(matrix)`
...
Note that "numeric" and "double" correspond to the same thing, difference is that "numeric" refers to the name of this specific formal class, and "double" refers to the name of the type. You can check what `typeof(10.0)` and `class(10.0)` give you.
...
Elements of a list must be named and you can retrieve one of these elements using the syntax `list$element`.
...
Note that matrix algebra in R does not strictly follow the rules of linear algebra, mostly due to its broadcasting feature. Sometimes you should also utilize this R-specific behavior to speed up computations.
...
For example, you could use `pnorm` function to compute the p-value of z-statistic of 2.1
...
A natural next step is answering: what if we need to execute another expression in the case the condition doesn’t hold? The else keyword is the answer:
...
In R, conditional statements specified in an if-else clause cannot have length > 1. If you need to check whether all elements of a logical vector are true, use `all(...)` function, or if any, use `any(...)`.
```{r}
all_true <- rep(TRUE, 10)
if (all(all_true)) "yes, all elements in all_true has truthy values" else "this won't be executed"

one_true <- c(rep(FALSE, 9), TRUE)
if (any(one_true)) "yes, there is at least one TRUE in one_true" else "this won't be executed"
```
...
curr.if("US")
```
but that is verbose, especially for a large number of cases.
...
Where x and y are the values (R objects) you want to pass to the function. By the way, it is not necessary to put the argument names into the function call. However, it can improve readability of your code. 

Lets see an example where we replicate the `seq` function with the default increment of 1
...
Now we can check what happens if we delete the `z` variable at all and add one more assignment statement for `x` (in function `i`).
...
It is worth mentioning how you can map your function to each element of a vector when it does not exhibit a pure "vectorization". For this purpose, there are functions like `lapply`, `sapply`, and `vapply`. They differ in accepted arguments and in a way they produce a result. For example, `lapply` tries to return a list, whereas `sapply` tries to simplify the result as much as possible. There is also just `apply` which is used more for matrices and arrays.

```{r}
vapply(1:3, function(x) x^2, double(length = 1L))

newMatrix = matrix(1:3, nrow = 3, ncol = 3)
newMatrix
